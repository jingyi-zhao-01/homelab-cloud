...
- **Observability Agent** (`flashsale/perf/python/observability_agent.py`): HTTP server deployed in its own namespace. Receives perf test failure events from GitHub Actions via POST and autonomously scaffolds analysis, Grafana queries, and PR creation.
