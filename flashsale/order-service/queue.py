...
def _load_stdlib_queue_module() -> Any:
    stdlib_dir = Path(sysconfig.get_paths()["stdlib"])
    queue_path = stdlib_dir / "queue.py"
    spec = spec_from_file_location("python_stdlib_queue", queue_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"unable to load stdlib queue module from {queue_path}")
