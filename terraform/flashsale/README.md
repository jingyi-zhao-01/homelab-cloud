...
- The Aiven Kafka SASL credential is intentionally managed outside Terraform because Aiven returns generated Kafka user passwords to Terraform as the literal placeholder `<redacted>`. Keep `/flashsales/prod/KAFKA_USERNAME` and `/flashsales/prod/KAFKA_PASSWORD` aligned with the real Aiven SASL credential currently in use. The current default username is `avnadmin` unless you explicitly override `aiven_kafka_order_service_username`.
